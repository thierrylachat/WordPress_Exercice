...
function kale_post_excerpts($more){
    $kale_posts_excerpt_read_more_text = kale_get_option('kale_posts_excerpt_read_more_text');
    $kale_posts_excerpt_read_more_link = kale_get_option('kale_posts_excerpt_read_more_link');
    if( empty ( $kale_posts_excerpt_read_more_text ) ) $kale_posts_excerpt_read_more_text = '&hellip;';
    
    if ( ! is_single() ) {
        if( $kale_posts_excerpt_read_more_link == 1 )
            $more = sprintf( '&nbsp;<a class="read-more" href="%1$s">%2$s</a>',
                get_permalink( get_the_ID() ),
                esc_attr ( $kale_posts_excerpt_read_more_text ) 
            );
        else 
           return esc_html ( $kale_posts_excerpt_read_more_text );
    }
    return $more;
}
